...
if [[ -n $bgcolor ]]; then
	...
	sed -r -i "s/(background-color-argb.*0x\w{2})\w{6}/\1$bgcolor/g" "$DIR/yabar/yabaromain.conf"
	sed -r -i "s/(background-color-argb.*0x\w{2})\w{6}/\1$bgcolor/g" "$DIR/yabar/yabarobin.conf"
fi
